  ...
  const handleSubmit = async (e) => {
    ...
    if (firstName && email && password) {
      const fetchData = await fetch(
        `${process.env.REACT_APP_SERVER_DOMAIN}api/signup`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="p-3 md:p-4 relative top-20 ">
      <div className="w-full max-w-sm bg-white m-auto flex items-center flex-col p-2 border-t-green-700 border-b-red-700 border-l-purple-600 border-r-yellow-400 border-5 rounded-2xl">
        <div className="w-20 h-20 overflow-hidden rounded-full border-2  border-green-500 drop-shadow-md shadow-md m-auto relative ">
          <img
        ...
        <form className="w-full py-3 flex flex-col" onSubmit={handleSubmit}>
          ...
          <div className="flex px-2 py-1 bg-slate-200 rounded mt-1 mb-2 focus-within:outline focus-within:outline-black-300">
            <input
              ...
              id="password"
              name="password"
              className="mt-1 mb-2 w-full bg-slate-200 rounded border-none outline-none"
              value={data.password}
              onChange={handleOnChange}
            />
            <span
          ...
          <button className="w-full max-w-[150px] m-auto  bg-green-500 hover:bg-red-500 cursor-pointer  text-white text-xl font-medium text-center py-1 rounded-full mt-4">
            Sign up
          </button>
        </form>
